...
# --- ИЗМЕНЕНА функция сравнения структур ---
def compare_structures(smiles1, smiles2):
    """Сравнивает две структуры через их InChIKey после санитизации."""
    if not smiles1 or not smiles2:
        return False
    
    mol1 = Chem.MolFromSmiles(smiles1, sanitize=True) # sanitize=True по умолчанию, но указываем для ясности
    mol2 = Chem.MolFromSmiles(smiles2, sanitize=True)
    
    if mol1 is None or mol2 is None:
        # Это может случиться, если пользователь нарисовал химически невозможную структуру
        return False
        
    # --- НОВОЕ: Явный вызов санитизации для максимальной надежности ---
    # Этот шаг помогает RDKit правильно "увидеть" ароматичность в структурах Кекуле
    try:
        Chem.SanitizeMol(mol1)
        Chem.SanitizeMol(mol2)
    except:
        # Если санитизация не удалась (например, неверная валентность), считаем структуры разными
        return False
